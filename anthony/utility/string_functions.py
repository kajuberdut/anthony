""" https://github.com/Rafiot/Stemming-Python/blob/master/stemming/porter2.py
...
def replacer(text: str, replacements: t.Dict[str, str], reverse=False) -> str:
    ...
    pattern = re.compile(
        "|".join([re.escape(key.lower()) for key in sorted_replacement_keys]),
        re.IGNORECASE,
    )
...
def tokenize(text: str) -> t.Tuple[int, str]:
    return [
        (i, w)
